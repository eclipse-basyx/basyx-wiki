...
| --- | --- | --- | --- |
| AAS, Submodel, and [Concept Description](../concept_description_repository/index) Repositories; AAS and Submodel Registries; Discovery Service; AAS Environment; [AASX File Server](../aasx_file_server/index) | OIDC and ABAC middleware | Set `abac.enabled: true` (or `ABAC_ENABLED=true`) and provide the trust list and policy configuration. | The active policy must cover that executable's routes and data. |
