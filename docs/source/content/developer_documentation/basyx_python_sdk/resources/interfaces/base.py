...
class XmlResponse(APIResponse):
    ...
    def serialize(self, obj: ResponseData, cursor: Optional[int], stripped: bool) -> str:
        ...
        xml_str = etree.tostring(
            root_elem, xml_declaration=True, encoding="utf-8")
...
class BaseWSGIApp:
    ...
    def _get_slice(cls, request: Request, iterator: Iterable[T]) -> Tuple[Iterator[T], int]:
        ...
        try:
            limit, cursor = int(limit_str), int(
                cursor_str) - 1  # cursor is 1-indexed
            if limit < 0 or cursor < 0:
                raise ValueError
        except ValueError:
            raise BadRequest(
                "Limit can not be negative, cursor must be positive!")
        start_index = cursor
...
class HTTPApiDecoder:
    ...
    def assert_type(cls, obj: object, type_: Type[T]) -> T:
        if not isinstance(obj, type_):
            raise BadRequest(
                f"Object {obj!r} is not of type {type_.__name__}!")
        return obj
    ...
    def json_list(cls, data: Union[str, bytes], expect_type: Type[T], stripped: bool, expect_single: bool) -> List[T]:
        ...
        try:
            ...
            if isinstance(parsed, list) and expect_single:
                raise BadRequest(
                    f"Expected a single object of type {expect_type.__name__}, got {parsed!r}!")
            if not isinstance(parsed, list) and not expect_single:
                raise BadRequest(
                    f"Expected List[{expect_type.__name__}], got {parsed!r}!")
            ...
            constructor: Optional[Callable[..., T]] = mapping.get(
                expect_type)  # type: ignore[assignment]
            args = []
    ...
    def request_body_list(cls, request: Request, expect_type: Type[T], stripped: bool) -> List[T]:
        ...
        if request.mimetype == "application/json":
            ...
            if isinstance(parsed, list):
                ...
                # type: ignore
                return [cls._convert_single_json_item(item, expect_type, stripped) for item in parsed]
            else:
...
def is_stripped_request(request: Request) -> bool:
    ...
    if extent is not None:
        raise werkzeug.exceptions.NotImplemented(
            f"The parameter extent is not yet implemented for this server!")
    return level == "core"
